...
function KakaoMap({ value, count }) {
  ...
  */
  ...
  const infowindow = new window.kakao.maps.InfoWindow({ zIndex: 1000 }); // 인포윈도우 변수 선언
  ...
  useEffect(() => {
    ...
    var map = new window.kakao.maps.Map(container.current, mapOption); // 지도를 생성해주고, 객체를 리턴합니다.
  ...
  return (
    <>
      <div
        className="map"
        style={{ width: "3000px", height: "500px", margin: "20px" }}
        ref={container}
      ></div>
